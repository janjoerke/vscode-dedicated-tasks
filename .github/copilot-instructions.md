...
1. **extension.ts** - Activation entry point, command registration, file watchers for `.vscode/tasks.json` and `.vscode/launch.json`
2. **tasksTreeProvider.ts** - Core logic: parses `dedicatedTasks` metadata from both files, builds unified hierarchical tree structure, provides tree data to VS Code's TreeView API
3. **statusBarManager.ts** - Manages status bar items based on user-selected tasks/launch configs/groups, persists configuration in `globalState`

**Key data flow**: File watchers detect changes → `tasksProvider.refresh()` → re-parses both JSON files → rebuilds unified hierarchy → updates both tree view and status bar
...
- **TaskTreeItem**: Wraps `vscode.Task`, executes via `vscode.tasks.executeTask()`
- **LaunchConfigItem**: Stores config name + workspace folder, executes via `vscode.debug.startDebugging()`
- Both share the same `DedicatedTasksConfig` interface and sorting/grouping logic
...
`buildHierarchy()` creates a unified tree from both tasks and launch configs:
- Merges items from `getDedicatedTasks()` and `getDedicatedLaunchConfigs()`
...
**Changing tree display logic**: Modify `buildHierarchy()` in tasksTreeProvider. Remember to preserve:
